...
const { createArtifact, createProcessedShotAnalysis, createContactSheet, waitForJob, createCountingProcessor } = require("./artifact-index.helpers");
...
test("failed shot boundary artifact does not register processed shot cache entry", async () => {
  ...
  const artifact = {
    ...
    shotBoundaryAnalysis: {
      ...
      agent: { provider: "codex-appserver", role: "shot-boundary-raw-analyze-legacy", skillPath: "SKILL.md", skillHash: "hash", threadId: "thread_1", leaseId: "lease_1", turnId: "turn_1" },
      shots: [],
      createdAt: new Date().toISOString(),
    },
  };
