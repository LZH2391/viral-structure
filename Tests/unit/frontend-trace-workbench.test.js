...
test("workbench defaults to new UI and persists new UI theme preference", () => {
  ...
  assert.match(newUi, /<NewUiLayout \/>/);
  ...
  assert.match(newUiLayout, /leftRatio: \{ min: 0\.2, max: 0\.4 \}/);
  assert.match(newUiLayout, /rightRatio: \{ min: 0\.2, max: 0\.4 \}/);
  ...
  assert.match(newUiLayoutCss, /border(?:-left|-right)?: 1px solid var\(--new-ui-border\)/);
  assert.doesNotMatch(newUiLayoutCss, /--new-ui-(?:ink|fog)-/);
