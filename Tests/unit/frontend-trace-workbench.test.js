const { test, assert, fs, path, vm, ts, read, readPropertyPanelCss, readWorkbenchApi } = require("./frontend-trace.helpers");
...
test("full analysis sync keeps atomization job independent and labels trace layers", () => {
  ...
  const fullLayout = read(root, "Apps/Workbench/src/components/full-analysis/FullAnalysisLayout.tsx");
...
test("embedded full analysis preserves running workflow during workbench artifact sync", () => {
  const root = path.resolve(__dirname, "../..");
  const full = read(root, "Apps/Workbench/src/components/FullAnalysisApp.tsx");
  const workflowModel = read(root, "Apps/Workbench/src/components/full-analysis/fullAnalysisWorkflowModel.ts");

  assert.match(workflowModel, /function shouldPreserveActiveWorkflow/);
  assert.match(workflowModel, /activeSample\.activeSampleSource === "fullAnalysis"/);
  assert.match(workflowModel, /return isRunExecuting\(run\)/);
...
test("full analysis includes optional atomization stage and cache surface", () => {
  ...
  assert.match(fullLayout, /label="原子化"/);
...
test("workbench defaults to new UI and persists new UI theme preference", () => {
  ...
  assert.match(newUi, /<NewUiLayout active=\{active\} theme=\{theme\} onThemeChange=\{onThemeChange\} onLeftCollapsedChange=\{onLeftCollapsedChange\} \/>/);
  ...
  assert.match(newUiLayout, /NEW_UI_THREE_PANE_STORAGE_KEY = "new-ui:three-pane-layout"/);
  assert.match(newUiLayout, /storageKey: NEW_UI_THREE_PANE_STORAGE_KEY/);
  ...
  assert.match(newUiLayout, /onLibraryChildChange=\{handleSidebarLibraryChildChange\}/);
  assert.match(newUiLayout, /setActiveLibraryChild\(child\)/);
  assert.match(newUiLayout, /new-ui-sidebar-subnav/);
  assert.match(newUiLayout, /new-ui-sidebar-subnav-item/);
  assert.match(newUiLayout, /expandedSections\.includes\(section\.id\)/);
  assert.match(newUiLayout, /isExpanded \? "is-expanded" : ""/);
  assert.match(newUiLayout, /setExpandedSections\(\(current\) =>/);
  assert.match(newUiLayout, /--new-ui-sidebar-subnav-expanded-height/);
  ...
  assert.match(newUiLayoutCss, /grid-template-columns: var\(--new-ui-left-pane-width\) var\(--new-ui-left-resize-width\) minmax\(0, 1fr\) var\(--new-ui-right-resize-width\) var\(--new-ui-right-pane-width\)/);
  ...
  assert.match(newUiLayoutCss, /\.new-ui-sidebar-nav-group\.is-expanded \.new-ui-sidebar-subnav/);
...
test("media preview uses ResizeObserver and preserves full aspect ratio metadata", () => {
  ...
  assert.match(helpers, /resolveAudioFeatureSourceId/);
