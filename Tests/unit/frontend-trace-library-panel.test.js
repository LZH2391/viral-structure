...
test("pixi graph colors are sourced from css graph tokens", () => {
  ...
  assert.match(graphCss, /--slot-graph-edge-muted-opacity: 0\.14/);
  assert.match(graphCss, /--slot-graph-node-muted-alpha: 0\.7/);
  assert.match(graphCss, /--slot-graph-node-focus-muted-alpha: 0\.1/);
  ...
  const previewPopoverBlock = graphCss.match(/\.new-ui-shell \.slot-graph-shell\.embedded \.slot-graph-preview-popover \{[\s\S]*?\n\}/)?.[0] ?? "";
  assert.match(previewPopoverBlock, /border: 1px solid var\(--new-ui-control-border\)/);
  assert.match(previewPopoverBlock, /background: var\(--new-ui-control-bg\)/);
  assert.match(previewPopoverBlock, /box-shadow: 0 10px 24px var\(--new-ui-control-shadow\)/);
  assert.match(previewPopoverBlock, /backdrop-filter: none/);
  assert.doesNotMatch(previewPopoverBlock, /rgba\(180, 170, 255|rgba\(20, 22, 30|backdrop-filter: blur/);
  assert.match(graphCss, /\.new-ui-shell \.slot-graph-shell\.embedded \.slot-graph-preview-head span \{[\s\S]*border-radius: 999px/);
  assert.match(graphCss, /\.new-ui-shell \.slot-graph-shell\.embedded \.slot-graph-preview-meta \{[\s\S]*border-top: 1px solid var\(--new-ui-border\)/);
  assert.match(graphCss, /\.new-ui-shell\[data-theme="light"\] \.slot-graph-shell\.embedded \.slot-graph-preview-popover \{[\s\S]*background: var\(--new-ui-surface\)/);
  ...
  assert.match(graphPixiCanvas, /function fitGraphViewport\(nodes: SimNode\[\], size: \{ width: number; height: number \}\): ViewportTransform/);
  assert.match(graphPixiCanvas, /pendingViewportFitRef\.current = true/);
  assert.match(graphPixiCanvas, /const nextViewport = fitGraphViewport\(nextNodes, canvasSizeRef\.current\)/);
  assert.doesNotMatch(graphPixiCanvas, /const nextViewport = \{ x: 0, y: 0, k: 1 \};[\s\S]*setViewport\(nextViewport\);[\s\S]*setHoveredNodeId\(null\)/);
  assert.match(graphPixiCanvas, /const applyAnimatedViewport = \(nextViewport: ViewportTransform\) => \{[\s\S]*viewportRef\.current = nextViewport;[\s\S]*setViewport\(nextViewport\);/);
  ...
  assert.match(graphPixiRenderer, /function safeDestroyText\(text: Text \| null\)/);
  assert.doesNotMatch(graphPixiRenderer, /view\.label(?:\?\.)?\.destroy\(\)/);
  assert.doesNotMatch(graphCss, /\.new-ui-shell \.slot-graph-shell\.embedded \.slot-graph-canvas\.structure\s*\{/);
  assert.doesNotMatch(graphCss, /\.slot-graph-canvas\.structure \.slot-graph-edge,\s*\.slot-graph-canvas\.plan-trace \.slot-graph-edge/);
  assert.doesNotMatch(graphCss, /\.slot-graph-canvas\.structure \.slot-graph-edge\.muted,\s*\.slot-graph-canvas\.plan-trace \.slot-graph-edge\.muted/);
  assert.doesNotMatch(graphCss, /\.slot-graph-canvas\.structure \.slot-graph-node\.muted,\s*\.slot-graph-canvas\.plan-trace \.slot-graph-node\.muted/);
  assert.match(graphCss, /\.slot-graph-canvas\.structure \.slot-graph-node\.focus-muted \{\s*opacity: var\(--slot-graph-node-focus-muted-alpha\);\s*\}/);
  assert.doesNotMatch(graphCss, /\.slot-graph-canvas\.plan-trace \.slot-graph-node\.node-sourceVariant\.node-layer-script[\s\S]*fill:/);
  ...
  assert.doesNotMatch(graphVisualStyles, /const traceLikeDimming = mode === "planTrace" \|\| mode === "structure"/);
  assert.doesNotMatch(graphVisualStyles, /mode === "structure"\) opacity = Math\.min\(opacity, 0\.36\)/);
  assert.doesNotMatch(graphVisualStyles, /mode !== "governance"\) return zoomFade\(zoom, 0\.45, 1, 0\.58, 1\)/);
  assert.match(graphVisualStyles, /function stagedZoomReveal\(zoom: number, start: number, end: number\)/);
  assert.match(graphVisualStyles, /if \(zoom < start\) return 0/);
  assert.match(graphVisualStyles, /if \(progress < 1 \/ 3\) return lerp\(0, 0\.1, progress \* 3\)/);
  assert.match(graphVisualStyles, /if \(progress < 2 \/ 3\) return lerp\(0\.1, 0\.6, \(progress - 1 \/ 3\) \* 3\)/);
  assert.match(graphVisualStyles, /return lerp\(0\.6, 1, \(progress - 2 \/ 3\) \* 3\)/);
  assert.match(graphVisualStyles, /const start = level === 0 \? 0\.72 : 0\.88 \+ \(level \* 0\.28\)/);
  assert.match(graphVisualStyles, /const end = start \+ 0\.6/);
  assert.match(graphVisualStyles, /function graphLabelSpec\(mode: GraphMode/);
  assert.match(graphVisualStyles, /if \(mode === "planTrace"\)[\s\S]*node\.type === "confirmedPlan"[\s\S]*node\.type === "sourceSample"/);
  assert.match(graphVisualStyles, /if \(node\.type === "libraryItem"\) return 0/);
  assert.match(graphVisualStyles, /const fontSize = Math\.max\(10, 15 - level\)/);
  assert.match(graphVisualStyles, /nodeMutedAlpha: 0\.7/);
  assert.match(graphVisualStyles, /nodeFocusMutedAlpha: 0\.1/);
  assert.match(graphVisualStyles, /edgeMutedOpacity: 0\.14/);
  assert.match(graphVisualStyles, /groupAlpha = theme\.state\.nodeMutedAlpha/);
  assert.match(graphVisualStyles, /classes\.has\("node-slot"\)[\s\S]*stroke = rgba\(theme\.node\.slotFamilyStroke, 1\)/);
  assert.doesNotMatch(graphVisualStyles, /classes\.has\("node-sourceVariant"\) && classes\.has\("node-layer-script"\)/);
  assert.match(graphVisualStyles, /classes\.has\("node-type-slotSubtype"\)[\s\S]*circleOpacity = 1/);
  assert.match(graphVisualStyles, /classes\.has\("node-script"\)[\s\S]*circleOpacity = 1[\s\S]*classes\.has\("node-rhythm"\)[\s\S]*circleOpacity = 1[\s\S]*classes\.has\("node-packaging"\)[\s\S]*circleOpacity = 1/);
  assert.match(graphVisualStyles, /className\.includes\("node-binding"\)[\s\S]*circleOpacity = 1/);
  assert.match(graphVisualStyles, /classes\.has\("node-sourceVariant"\)[\s\S]*circleOpacity = 1/);
  ...
  assert.match(graphPixiRenderer, /whiteTextStyle\(SLOT_BADGE_FONT_SIZE, theme\)/);
  assert.match(graphPixiRenderer, /view\.slotBadgeText\.position\.set\(0, 1\.25\)/);
});
