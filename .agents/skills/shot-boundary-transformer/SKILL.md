...
- 切出 shots 后，系统会另起视觉摘要 turn，提供按 shot 对齐的 `localImage` 镜头联表，以及该 shot 对齐的 `subtitleText / subtitleContextText`，用来修正 `shots[].summary`
...
视觉摘要 turn 可以用字幕辅助识别画面中可见对象、包装文字、商品名或身体部位，但不得把字幕里的 hook、卖点、价格、观点、口播目的、脚本段落职责或转化任务写进 `shots[].summary`。
