...
- 顶层必须有 `commerceBrief`
- 顶层必须有 `videoSummary`
- `shots[].summary` 只表示画面是什么，不得超过 15 字，10 字以内为好
- 最后一镜 `endBoundary` 必须为 `null`
- `commerceBrief` 继续兼容现有字段，不要删字段

`shots[].summary` 必须是短名词短语，不写完整句，不罗列多个细节。不得写 hook、话题、卖点、价格、观点、字幕语义、口播目的、脚本段落职责或转化任务。这些属于 `script-segment-analyzer` 的分析边界。
...
- `shots[].summary` 写成 UI 标签级短语，例如“人物躺床”“拉扯床单”“床单局部”“人物翻身”“产品包装”。
- 不要写“人物站在床边拉扯灰色格纹床单，床边可见多个卡扣”这类长句。
