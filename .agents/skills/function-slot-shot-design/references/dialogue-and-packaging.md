...
- `existing_material`：必须使用原素材镜头的字幕/口播；原素材无字幕/口播时写“无”，不得新写。
...
台词机器人感终审交给 `function-slot-dialogue-robotic-reviewer`。ShotDesign 只负责生成可执行初版台词，不在本文件内做机器人感 review 或整版台词回修。
