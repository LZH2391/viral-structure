...
- `包装说明` 写后期叠加的覆盖层、字幕、标题条、圈选、箭头、标签、图卡、画中画等，必须来自第 7 节包装证明方案；可写简单包装出现方式，例如直接出现、淡入、随动作弹出、简单依次落位。不要写花哨飞入、多层路径动画、三维旋转、粒子爆开等与证明无关的动效。凡是写主字幕样式，必须同时写出该 shot 真实会出现的主字幕文案，例如“底部主字幕：'先看这个膜感'，白字黑描边，关键词'膜感'黄色高亮”；不能只写纯样式。`包装说明` 只写画面可见覆盖层和剪辑执行规格，不得追加“目的：”“服务功能：”“证明功能：”等内部说明，也不得把“商品记忆点”“购买理由”“商品锚定”“证据标注”等结构词写成可见标签。
...
- `包装说明` 必须写“新包装/字幕如何在原素材画面上补强”，包括覆盖层位置、样式、出现时机和避让区域；这些内容服务剪辑后期，不服务生图。补强服务功能写入 `证明功能` 或 `必须同步点`，不要写成 `包装说明` 中的“目的：...”尾注。
