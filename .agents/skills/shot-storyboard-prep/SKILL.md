...
description: 从 function-slot-shot-design 产出的 shot-design.final.md 中提取 Shot 设计表，回填预计时长，并按每 4 镜头生成故事板生图准备 Markdown。用于用户要求把分镜画面和包装说明合成故事板/生图提示词、提取横竖屏画幅、运行 estimate_dialogue_duration.py 回填预计时长、或生成可喂给 ImgGen/PPAPI 的 storyboard prompts 时；不负责补写 Shot 设计，若缺少 shot-design.final.md，先使用 function-slot-shot-design。
---
