...
def _is_allowed_sample_dir(sample_dir: Path, search_root: Path) -> bool:
    ...
    seed_root = bundled_seed_root_for(sample_dir)
    if seed_root is None:
        return True
    return search_root == seed_root or is_relative_to(search_root, seed_root)


def bundled_seed_root_for(path: Path) -> Optional[Path]:
    """Return the owning skill's seed-library root when path is inside one."""
    resolved = path.resolve()
    for parent in [resolved, *resolved.parents]:
        if parent.name != "sample-libraries" or parent.parent.name != "references":
            continue
        parts = parent.parts
