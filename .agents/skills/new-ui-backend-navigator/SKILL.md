...
- 允许在合适时机主动运行构建命令（如 `npm run build:*`、`vite build`、`tsc --noEmit` 等），但必须克制使用，避免高频重复构建。
- 构建优先用于：前端入口/路由/类型合同/打包产物相关改动完成后、源码检查无法覆盖风险时、或需要确认当前页面实际可用时。
- 一轮任务内如果已经完成过相关构建，后续小改优先跑定向测试或轻量静态检查；除非改动触及新的构建风险，不要反复构建。
- 禁止主动使用 Playwright、浏览器自动化、页面截图或浏览器实测检查。
- 只有当用户明确要求“跑 Playwright / 浏览器检查 / 截图检查 / 实测页面”时，才运行对应浏览器实测检查。
- 如果完成改动后未运行构建或浏览器检查，需在 final 中说明原因，并给出用户可手动运行的命令建议。
