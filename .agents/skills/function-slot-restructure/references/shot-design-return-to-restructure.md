...
当 ShotDesign 判断问题不是“少几个镜头”，而是当前槽位链整体不适合当前素材、合理包装字幕和 AIGC 补镜头时，触发：
...
- 无法落地原因：现有素材不足、AIGC 不能安全补、证明会被伪造、复用压力过高、顺序/节奏不成立等。
- 已排除的策略：现有素材、包装字幕强化、AIGC 自行设计、复用变形兜底。
...
- 如果只是单个镜头缺失、承接画面不足或 CTA 画面不足，优先由 ShotDesign 自行设计 AIGC 镜头，不触发回重组。
