...
- **platform and production constraints / 平台和生产约束**：平台限制、信息密度、创作者风格、CTA 强度要求。Brief 可以提供 `targetDurationSec` 作为粗略目标时长，建议按 15 秒步长填写，例如 `15 / 30 / 45 / 60`；它只作为后置预估参考，不在重组阶段拆成时间码、shot 时长或段落时长。
...
## 步骤 9：并行设计脚本段落、节奏曲线和包装证明

只有在需求图、链路假设、槽位实现和 adapters 稳定后，才进入表达层落地。

脚本段落、节奏曲线和包装证明是三条并行视图，不是线性派生关系：

- 脚本段落回答语义如何分段推进，承接 script atoms 的主张、证明义务和前后依赖。
- 节奏曲线回答注意力如何变化，承接 rhythm atoms 的速度、密度、停顿、峰值和同步要求。
- 包装证明回答主张如何被看见和相信，承接 packaging atoms 的证明功能、视觉层级和可替换载体。

不要使用“脚本节拍”作为结构单位。脚本段落可以一对一、一对多或多对一承接 script atoms。
...
不要在重组阶段提前设计时间码、时间轴位置或段落时长分配；只写顺序、承载关系、节奏状态和同步关系。Brief 里的 `targetDurationSec` 只能作为粗略目标和后置预估参考，不直接转成槽位数量、shot 数量、合并策略或具体时间安排。

唯一例外发生在 Shot 设计阶段：无台词镜头，或镜头内无台词停顿/动作展示段，必须说明该无台词部分的近似时长，执行误差不得超过 1 秒。这个时长只服务分镜可执行性，不能反向决定槽位链、shot 数量、脚本段落、节奏区间或包装方案。

台词已经写好后，可以使用 `scripts/estimate_dialogue_duration.py` 按默认 `6 字/秒` 做后置估算。该估算只用于检查台词长度风险，不反向决定槽位链、shot 数量、分镜结构或包装设计。

总时长预估发生在 Shot 设计之后：用台词估算时长加上第 8 节填写的无台词时长，与 brief 的 `targetDurationSec` 做大致对齐。若明显偏离目标，只在剩余风险与修复中提出压缩、删减、合并或改写建议；不要回头把目标时长当作结构生成器。

设计时按新视频重新编号，例如 `new_shot_01`、`new_shot_02`。每个 shot 或 shot group 至少要说明：

- 覆盖哪个槽位或 adapter。
- 承载哪个脚本段落、节奏区间和包装块。
- 具体分镜画面：画面主体、动作、屏幕录制、产品露出、结果或证明如何出现。
- 台词/字幕（若有）：只承接第 5 节脚本段落方案，不临时新增偏离主张。
- 无台词时长：无台词镜头或镜头内无台词停顿/动作展示段必须填写，误差不得超过 1 秒；没有无台词段则写“无”。
- 包装说明：只承接第 7 节包装与证明方案，不临时新增装饰性包装。
- 哪些主张、动作、结果、图文证据、字幕强调或包装弹出必须同步。
- 当前 shot 满足哪类证明功能。
