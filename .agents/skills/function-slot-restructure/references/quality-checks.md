...
按 `SKILL.md` 的“台词与字幕原则”逐条检查第 8 节台词/字幕；不合格时直接回到第 8 节重写，不把它降级成第 10 节风险提示。

## 包装 / 字幕规格检查

按 `SKILL.md` 的“包装与字幕默认层”逐条检查第 7、8 节：

- 第 7 节每个包装块是否写了字幕层规格，或明确说明无字幕及原因？
- 字幕是否写清功能、样式、位置、强调规则、出现时机和避让要求？
- 是否避免了只写“短字幕、轻量字幕、极简 CTA、短标签”这类不可执行描述？
- 特殊字、关键词、数据、证据标签是否说明了不同样式，例如颜色、描边、底色、胶囊标签、框选或高亮？
- 字幕层是否服务证明、视线引导、动作命名、场景归类或 CTA，而不是只做装饰？
- 字幕、标签、箭头、圈选、图卡是否不会遮挡主体细节、证据区域、关键动作、结果状态或人物表情？
- 第 8 节每个 shot 的包装说明是否继承第 7 节字幕层规格，而不是临时新增或漏写？
