...
## 切镜指导
联表每格为一个采样帧，格内文字为字幕，格下为 frameId 和时间戳。

判断切镜依据视觉变化：背景、人物位置、景别、光线出现明显不连续即为边界。字幕内容不作为切镜依据。

相邻两格视觉高度相似则视为同一镜头内的连续采样，不输出 boundary。

联表按 sheetIndex 顺序阅读，跨 sheet 的相邻帧（overlapFrameIds）需连续比对，不要在 sheet 边界处强制切分。
