...
import { attachAnalysisJob, runRhythmStructureAnalysis, runScriptSegmentAnalysis } from "../utils/workbenchHelpers";
import { writeActiveAnalysisJob } from "../utils/workbenchDraft";

type AnalysisKind = "scriptSegment" | "rhythmStructure";
...
export function useAnalysisJobFlow(options: FlowOptions) {
  ...
  const run = useCallback(async (cacheDecision: "ask" | "refresh" = "ask") => {
    const runner = kind === "scriptSegment" ? runScriptSegmentAnalysis : runRhythmStructureAnalysis;
    const result = await runner(
      ...
      async ({ job: waitingJob, cachedItem }) => {
        ...
        setSaveStatus(kind === "scriptSegment" ? "命中脚本段落缓存，等待选择" : "命中节奏结构缓存，等待选择");
      },
      cacheDecision,
    );
    return result;
