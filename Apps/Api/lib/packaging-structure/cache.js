...
async function findCachedArtifact({
  ...
  const params = buildPackagingStructureCacheParams({
  ...
  const analysis = artifact?.packagingStructureAnalysis ?? null;
...
async function resolveCachedPrompt({ cachePrompt, artifactIndex, evaluateCacheEligibility, codedError, expectedCacheKey }) {
  ...
  if (!sampleVideoId) throw codedError("packaging_structure_cache_source_missing", "包装结构缓存来源缺失，请重新生成", null, false);
  ...
  if (!cacheEligibility.eligible) {
    throw codedError("packaging_structure_cache_not_reusable", "包装结构缓存不可复用，请重新生成", { eligibility: cacheEligibility }, false);
  }
  return {
    cache: {
      sampleVideoId,
...
function buildCachedItem(context, cached) {
  return {
    ...
    sourceCreatedAt: cached.analysis?.createdAt ?? null,
    cacheKey: context.cacheKey ?? cached.cache.cacheKey ?? null,
    packagingBlockCount: cached.analysis?.packagingBlocks?.length ?? 0,
    shotPackagingNoteCount: cached.analysis?.shotPackagingNotes?.length ?? 0,
