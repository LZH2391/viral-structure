...
function buildProcessedAnalysisFromParsed(parsed, prepared, contactSheets, context, lease, turn, options = {}) {
  ...
  return {
    ...
    agent: {
      ...
      role: options.agentRole ?? ROLE,
      profilePath: options.agentProfilePath ?? context.roleProfile?.profilePath ?? null,
      profileVersion: options.agentProfileVersion ?? context.roleProfile?.profileVersion ?? null,
      promptTemplateId: options.agentPromptTemplateId ?? context.promptTemplate?.promptTemplateId ?? null,
      promptTemplateVersion: options.agentPromptTemplateVersion ?? context.promptTemplate?.promptTemplateVersion ?? null,
      promptTemplateHash: options.agentPromptTemplateHash ?? context.promptTemplate?.promptTemplateHash ?? null,
      initFingerprint: options.agentInitFingerprint ?? context.initFingerprint ?? null,
      skillPath: options.agentSkillPath ?? context.skillPath ?? SKILL_PATH,
      skillHash: options.agentSkillHash ?? context.skillHash ?? skillContentHashSync(context.skillPath ?? SKILL_PATH),
      threadId: lease.thread_id,
      leaseId: lease.lease_id ?? null,
      turnId: turn.turnId,
      sheetCount: contactSheets.length,
      inputMode: options.inputMode ?? "multi_contact_sheet",
      enableReview: options.enableReview !== false,
      reviewMode: options.enableReview === false ? "unreviewed" : "reviewed",
      rawAnalyzer: options.rawAnalyzer ?? null,
    },
    shots,
