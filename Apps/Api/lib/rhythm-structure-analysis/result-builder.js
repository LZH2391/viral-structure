...
function buildProcessedAnalysis(message, input, context, agentRun, turn, { repairAttemptCount = 0 } = {}) {
  ...
  return {
    ...
    cacheKey: context.cacheKey ?? buildRhythmStructureContentFingerprint(input, context.inputPackage),
...
function buildFailedArtifact(context, errorSummary, debugSnapshotUri = null) {
  return {
    ...
    cacheKey: context.cacheKey ?? (context.input ? buildRhythmStructureContentFingerprint(context.input, context.inputPackage) : null),
