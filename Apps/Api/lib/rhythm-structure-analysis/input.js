...
const INPUT_PACKAGE_SCHEMA_VERSION = "rhythm_structure_input_package.v1";
const RHYTHM_STRUCTURE_SHEET_PURPOSE = "rhythm_structure_shot_context";
const RHYTHM_STRUCTURE_SHEET_SUBDIR = "rhythm-structure-shot-sheets";
...
function prepareInput(artifact, options = {}) {
  ...
  if (!shots.length) {
    throw codedError("rhythm_structure_missing_shots", "当前样例没有可分析的切镜结果", null, false);
  }
...
async function prepareInputPackage({ input, sampleDir, store }) {
  if (!sampleDir || !store) throw new Error("rhythm structure input package missing sampleDir/store");
  const inputPackageDir = path.join(sampleDir, "rhythm-structure-input");
...
async function buildVisualManifest({ input, shotFramePages, sampleDir, sheetsDir, store }) {
  return sanitizeForAppServerText(await buildShotFrameVisualManifest({
    ...
    store,
    schemaVersion: INPUT_PACKAGE_SCHEMA_VERSION,
    sheetPurpose: RHYTHM_STRUCTURE_SHEET_PURPOSE,
  }));
}
...
function buildInputSummaryText(inputPackage) {
  const subtitleReadyShotCount = Array.isArray(inputPackage?.manifest?.shots)
    ...
    : 0;
  return `本次包含 ${inputPackage.manifest.shotCount} 个镜头、${inputPackage.visualManifest.sheetCount} 个代表帧联表页、${inputPackage.visualManifest.emptyShotCount} 个缺代表帧镜头；其中 ${subtitleReadyShotCount} 个镜头附带对齐字幕。输入包路径见下。`;
}
...
module.exports = {
  ...
  RHYTHM_STRUCTURE_SHEET_PURPOSE,
  RHYTHM_STRUCTURE_SHEET_SUBDIR,
  prepareInput,
  buildTurnInputs,
  prepareInputPackage,
