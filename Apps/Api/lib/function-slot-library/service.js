...
const { enrichRhythmAtomsWithTimingEvidence } = require("./rhythm-timing-evidence");
...
function buildLibraryPayload({ artifact, analysis, exportedAt }) {
  ...
  const rhythmAtoms = enrichRhythmAtomsWithTimingEvidence(analysis.atomInventory?.rhythmAtoms ?? [], artifact);
