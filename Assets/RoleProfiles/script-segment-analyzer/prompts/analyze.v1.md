...
返回前自检：JSON 可解析；segments 按时间顺序且完整覆盖所有 shots；切段依据是表达任务变化而非固定段数；shotRefs 只引用输入中的 shotId；label 不是固定枚举；roleInScript 说明具体说服职责而不是空泛“承接表达”；transferableRule 只总结结构规则；不要输出解释性正文、本地路径、完整原文或新内容脚本。
