...
返回前自检：JSON 可解析；commerceBrief 六个字段都存在；如果没有明显转化动作，conversionAction 直接写“未观察到明显转化动作”；uncertainties 必须为数组；shots 必须按时间升序；第一镜 start 必须为 0；最后一镜 end 必须等于 durationSeconds；除最后一镜外每个 shot.endBoundary.timestamp 必须等于该镜 end；下一镜 start 必须等于上一镜 end；summary 描述“这一镜是什么”；reason 描述“为什么在这里切”；不要输出时间戳文本、本地路径、frameId、OCR 原文或解释性正文。
