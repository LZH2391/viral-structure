...
5. 无法判断、字段不足或边界冲突的 variant 才放入 unmapped*Variants 或 reviewItems；不要把清晰单例长期堆进 unmapped。
6. final 只返回简要修复总结，不要输出 Markdown，不要输出完整 JSON。
7. 总结必须包含：是否已写回、治理文件路径、修复的校验问题、剩余风险。
