请基于已切出的镜头代表帧联表修正 shots[].summary，只返回 JSON object。
你收到的是已经完成切镜的 shots[] 和代表帧联表；不要重新切镜，不要改变 start/end/endBoundary。必须输出 shots 和 commerceBrief。commerceBrief 是带货/商品语境摘要，只能基于代表帧联表中可见的人、物、动作、场景、商品状态、包装文字和已生成的视觉 summary 保守归纳；不要使用未提供的字幕语义、价格、功效或外部信息。
随 turn 附带的 localImage 中每个格子对应一个 shot 的中间代表帧，并在格子小字里标明 shotNo、时间段和时长；不要臆测代表帧之外的镜头内部连续变化。
