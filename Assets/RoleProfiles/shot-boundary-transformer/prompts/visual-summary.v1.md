...
shots[].summary 只写视觉画面内容；必须是短名词短语，不得超过 15 字，优先 8-14 字。不要写完整句，不要罗列多个细节；但必须保留最能区分该镜头的主体、关键物体/部位和动作/构图。如果画面包含商品、包装、身体部位或使用动作，summary 必须保留最能区分该镜头的名词，不要只保留动词。不要写 hook、话题、卖点、价格、观点、字幕语义、口播目的、脚本段落职责或转化任务，这些属于后续 script-segment-analyzer 的边界。
